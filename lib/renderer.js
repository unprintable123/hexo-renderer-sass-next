...
module.exports = function (data) {
  ...
  const opt = Object.assign({
    functions: {
      'url_for($str)': args => { return new sass.SassString(url_for.call(self, args[0].assertString('str').text)) },
      "hexo-theme-config($ckey)": function (ckey) {
        var val = getProperty(self.theme.config, ckey.text);
        return new sass.SassString(val);
      },
      "hexo-config($ckey)": function (ckey) {
        var val = getProperty(self.config, ckey.text);
        return new sass.SassString(val);
      }
    }
